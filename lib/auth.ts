 ...
 */
export async function createSessionAndToken(userId: mongoose.Types.ObjectId, ua?: string, ip?: string) {
  await connect();
  ...
  const session = await Session.create({ userId, ua, ip, active: true, expiresAt });
...
export async function verifyTokenAndSession(token: string) {
  ...
  try {
    ...
    if (!session || !session.active) throw new Error("Session invalid or logged out");
    if (session.expiresAt && session.expiresAt < new Date()) throw new Error("Session expired");

    const user = await User.findById(userId).lean();
    if (!user || !(user as any).isActive) throw new Error("User disabled or not found");
